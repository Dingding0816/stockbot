...
def run_prediction(symbol="MU", return_dict=False):
    try:
        ...
        if df_1m is not None and is_valid_kline(df_1m):
        ...
        def get_realtime_price():
            try:
                ...
                if trades:
                    return safe_value(trades[-1]["p"], prev_close)
            except:
                pass

            try:
                df_1m_local = get_1m_klines()
                if df_1m_local is not None and not df_1m_local.empty:
                    return safe_value(df_1m_local["c"].iloc[-1], prev_close)
            except:
                pass

            try:
                quote_symbol = finnhub_client.quote(symbol)
                return safe_value(quote_symbol.get("c"), prev_close)
            except:
                pass

            return safe_value(prev_close, 1.0)
        ...
        fallback_price = safe_value(prev_close if 'prev_close' in locals() else 1.0, 1.0)

        if return_dict:
            return {
                "symbol": symbol,
                "current_price": float(fallback_price),
                "best_buy_5m": float(fallback_price),
                "best_sell_5m": float(fallback_price),
                "true_high15": float(fallback_price),
                "true_low15": float(fallback_price),
                "true_high_full": float(fallback_price),
                "true_low_full": float(fallback_price),
                "est_high15": float(fallback_price),
                "est_low15": float(fallback_price),
                "est_high_full_day": float(fallback_price),
                "est_low_full_day": float(fallback_price),
                "extra_spread_pct": 0.0,
                "predicted_score": 0.0,
                "actual_result": 0.0,
                "weights": {},
