...
def load_push_count():
    try:
        ...
        data = {"month": datetime.datetime.now().strftime("%Y-%m"), "count": 0}
...
def run_prediction():
    ...
    def update_daily_ohlc():
        ...
        df = pd.DataFrame({
            "date": [datetime.datetime.fromtimestamp(t).strftime("%Y-%m-%d") for t in ts],
            "open": opens,
            "high": highs,
            "low": lows,
            "close": closes
    ...
    def get_1m_klines():
        ...
        df["datetime"] = df["t"].apply(lambda x: datetime.datetime.fromtimestamp(x))
